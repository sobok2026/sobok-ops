# Binds vibe.sobok.cc to the `vibe` Worker (Workers Static Assets deployment).
#
# The Worker script + its static assets are deployed by CI via `wrangler deploy`
# (see .github/workflows/vibe-deploy.yml + apps/vibe/wrangler.jsonc in the sobok repo);
...
# needed.
#
# ORDER: the `vibe` service must exist before this applies, so run the first CI deploy
